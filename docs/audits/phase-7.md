...
- **No fabricated principal direction.** The "PC direction" returned to the frontend is the largest-eigenvalue eigenvector of the _true_ input covariance matrix (computed via `np.linalg.eigh(cov)`), not the learned weight. The angle reported is angle between learned `w` and that true PC.
